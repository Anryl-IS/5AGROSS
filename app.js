...
const FEB_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTHNgLs14x9JzMkkOW8eQHCeXon0n_rsjFVuDHymx2mmgkB5WV8_WkOvkF9cvmAuE8N9usedhTdqbH8/pub?gid=130106578&single=true&output=csv';
...
async function fetchSyncData() {
    ...
    try {
        ...
        const [febRaw, marRaw] = await Promise.all([
            fetchFile(FEB_CSV_URL),
            fetchFile(MAR_CSV_URL)
        ]);

        const febData = parseSyncCSV(febRaw);
...
function renderSyncResults(feb, mar) {
    ...
    const rows = Object.values(map).sort((a, b) => b.mar - a.mar);
